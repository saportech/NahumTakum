...
void ESPNowMaster::onDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len) {
    ...
    if (slaveID != -1) {
        ...
        // Set data in sensorData
        sensorData[slaveID - 1].raw_pitch = raw_pitch; // Adjust index for 0-based array
        sensorData[slaveID - 1].yaw = yaw; // Adjust index for 0-based array
